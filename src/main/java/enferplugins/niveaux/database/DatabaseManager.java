package enferplugins.niveaux.database;
...
public class DatabaseManager {
    ...
    public DatabaseManager(NiveauxPlugin plugin) { this.plugin = plugin; }
    ...
    public boolean init() {
        ...
        config.setConnectionTimeout(30_000); config.setIdleTimeout(600_000); config.setMaxLifetime(1_800_000);
        ...
        try {
            ...
            plugin.getLogger().log(Level.SEVERE, "Impossible de se connecter à MySQL ! Vérifiez config.yml.", e);
            return false;
        }
    }
    ...
    public PlayerData loadPlayer(UUID uuid, String name) {
        ...
        try (Connection conn = dataSource.getConnection(); PreparedStatement ps = conn.prepareStatement(sql)) {
            ...
            try (ResultSet rs = ps.executeQuery()) {
                if (rs.next()) return new PlayerData(uuid, rs.getString("pseudo"), rs.getInt("niveau"), rs.getInt("temps_minutes"));
            }
        } catch (SQLException e) {
            plugin.getLogger().log(Level.SEVERE, "Erreur lors du chargement du joueur " + name + " (" + uuid + ")", e);
        }
        return new PlayerData(uuid, name, 0, 0);
    }

    public PlayerData loadPlayerByName(String name) {
        final String sql = "SELECT uuid, pseudo, niveau, temps_minutes FROM " + TABLE + " WHERE LOWER(pseudo) = LOWER(?)";
        try (Connection conn = dataSource.getConnection(); PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setString(1, name);
            try (ResultSet rs = ps.executeQuery()) {
                if (rs.next()) return new PlayerData(UUID.fromString(rs.getString("uuid")), rs.getString("pseudo"), rs.getInt("niveau"), rs.getInt("temps_minutes"));
            }
        } catch (SQLException e) {
            plugin.getLogger().log(Level.SEVERE, "Erreur lors de la recherche du joueur par pseudo : " + name, e);
        }
        return null;
    }

    public void savePlayer(PlayerData data) {
        final String sql = "INSERT INTO " + TABLE + " (uuid, pseudo, niveau, temps_minutes) VALUES (?, ?, ?, ?) ON DUPLICATE KEY UPDATE pseudo = VALUES(pseudo), niveau = VALUES(niveau), temps_minutes = VALUES(temps_minutes)";
        try (Connection conn = dataSource.getConnection(); PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setString(1, data.getUuid().toString()); ps.setString(2, data.getName());
            ps.setInt(3, data.getLevel()); ps.setInt(4, data.getMinutes());
            ps.executeUpdate();
        } catch (SQLException e) {
    ...
    public void saveAllPlayers(Collection<PlayerData> players) {
        ...
        try (Connection conn = dataSource.getConnection(); PreparedStatement ps = conn.prepareStatement(sql)) {
            ...
            plugin.getLogger().log(Level.SEVERE, "Erreur lors de la sauvegarde globale des joueurs", e);
        }
    }
