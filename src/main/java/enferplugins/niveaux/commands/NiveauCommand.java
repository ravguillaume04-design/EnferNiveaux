...
public class NiveauCommand implements CommandExecutor, TabCompleter {
    ...
    private void modifyData(CommandSender sender, String targetName, Consumer<PlayerData> modifier) {
        ...
        Bukkit.getScheduler().runTaskAsynchronously(plugin, () -> {
            ...
            Bukkit.getScheduler().runTask(plugin, () -> {
                if (data == null) { sender.sendMessage(ColorUtil.colorize("&#FF5555Joueur &#FFFFFF" + targetName + " &#FF5555introuvable en base de données.")); return; }
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String alias, String[] args) {
        ...
        if (args.length == 1) return ACTIONS.stream().filter(a -> a.startsWith(args[0].toLowerCase())).toList();
    ...
    private List<String> onlinePlayers(String prefix) {
        return Bukkit.getOnlinePlayers().stream().map(Player::getName).filter(n -> n.toLowerCase().startsWith(prefix.toLowerCase())).toList();
    }
}
